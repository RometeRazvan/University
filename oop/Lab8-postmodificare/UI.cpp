...
const void UI::afiseazaMap() {
	...
	for (unsigned int i = 0; i < myMap.size(); ++i) {
		pair<string, int> paiR = myMap.at(i);
...
const void UI::afiseaza() {
	...
	for (unsigned int i = 0; i < size; ++i) {
		...
		vector<string> vect;
		vect.push_back(locatar.getApartament());
		vect.push_back(" ");
		vect.push_back(locatar.getNume());
		vect.push_back(" ");
		vect.push_back(locatar.getSuprafata());
		vect.push_back(" ");
		vect.push_back(locatar.getTip());
		vect.push_back("\n");

		string str = accumulate(vect.begin(), vect.end(), string{});
