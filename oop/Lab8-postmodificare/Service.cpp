...
pair<string, int> func(pair<string, int> p) {
	return pair<string, int>(p.first, p.second);
}

vector<pair<string, int>> Service::makeMap() {
	...
	transform(myMap.begin(), myMap.end(), back_inserter(vec), func);
...
MyList<Locatar>& Service::getList() {
	return repo.getList();
}